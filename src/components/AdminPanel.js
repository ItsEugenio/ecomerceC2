...
function AdminPanel() {
  ...
  const handleAddProduct = () => {
    // Verificar si los campos necesarios están completos antes de enviar la solicitud POST
    if (newProduct.name && newProduct.price > 0 && newProduct.image) {
      // Enviar solicitud POST para agregar un nuevo producto
      ...
      // Manejo de errores o validación si faltan datos
      console.error(
  ...
  const handleDeleteProduct = (productId) => {
    // Enviar solicitud DELETE para eliminar el producto
    axios
            ...
            <>
              {editingProduct && editingProduct.id === product.id ? (
                <div className={styles.contEditar}>
                  ...
                  <Button color="success" outline onClick={() => handleSaveClick(editingProduct)}>
                    Guardar
                  </Button>
                </div>
              ) : (
